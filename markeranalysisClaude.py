...
trials = [
    {
        ...
        "neon_fps": 30,               # scene camera frame rate
...
# ─────────────────────────────────────────────
# ANALYSIS — matching colleague's exact method
# ─────────────────────────────────────────────
...
for trial in trials:
    ...
    # --- Step 2 & 3 (colleague): first eye frame UTC timestamp ---
    recording = nr.open(trial["neon_recording_path"])
    first_eye_frame_utc_ms = recording.eye.time[0] / 1e6  # microseconds → milliseconds

    # --- Step 4a (colleague): impact time on PHONE clock ---
    # first_eye_frame_utc_ms + (frame_of_impact * ms_per_frame)
    ...
    # --- Step 5 (colleague): convert to PC clock by adding time offset ---
    # equivalent to colleague's: + laptop_to_neon_time_offset_ms
    impact_time_pc_neon_ms = impact_time_phone_ms + phone_to_pc_time_offset_ms

    # --- Step 6 (colleague): QTM impact time on PC clock ---
    # equivalent to colleague's: laptop_time_mocap_start_ms + (FRAME_NUM * 8.333)
    ...
    # --- Step 7 (colleague): offset = qtm_impact - neon_impact ---
    # Negative = Neon sees the event LATER than QTM
    ...
    results.append({
        ...
        "first_eye_frame_utc_ms":    round(first_eye_frame_utc_ms, 4),
        "impact_time_phone_ms":      round(impact_time_phone_ms, 4),
        "impact_time_pc_neon_ms":    round(impact_time_pc_neon_ms, 4),
        "impact_time_pc_qtm_ms":     round(impact_time_pc_qtm_ms, 4),
        "offset_ms":                 round(offset_ms, 4),
    })

    print(f"[{trial_id}]")
    print(f"  First eye frame UTC  : {first_eye_frame_utc_ms:.4f} ms")
    print(f"  Impact - phone clock : {impact_time_phone_ms:.4f} ms")
    print(f"  Impact - PC/Neon     : {impact_time_pc_neon_ms:.4f} ms")
    print(f"  Impact - PC/QTM      : {impact_time_pc_qtm_ms:.4f} ms")
    print(f"  Offset (QTM - Neon)  : {offset_ms:.4f} ms\n")
...
csv_path = "sync_offset_results.csv"
fieldnames = [
    "trial_id", "subject_id", "task_name", "trial_number",
    "first_eye_frame_utc_ms", "impact_time_phone_ms",
    "impact_time_pc_neon_ms", "impact_time_pc_qtm_ms", "offset_ms"
]
