...
async def send_test_trigger():
    ...
    qtm_task = asyncio.create_task(
        qtm_connection.set_qtm_event("TEST_TRIGGER")
    ) if qtm_connection else None

    neon_task = asyncio.create_task(
        asyncio.to_thread(neon_device.send_event, "TEST_TRIGGER")
    ) if neon_device else None

    tasks_to_run = [t for t in [qtm_task, neon_task] if t is not None]

    if tasks_to_run:
        results = await asyncio.gather(*tasks_to_run, return_exceptions=True)
        for label, result in zip(["QTM", "Neon"], results):
            if isinstance(result, Exception):
                print(f"Failed to send {label} trigger: {result}")
            else:
                print(f"{label} trigger sent")
# ------------------ QTM SAVE & RESET ------------------
...
async def start_recording_and_trial():
  ...
  if neon_device:
      try:
          # Estimate offset BEFORE recording starts, averaged over 5 samples
          try:
              samples = []
              for _ in range(5):
                  offset_result = await asyncio.to_thread(neon_device.estimate_time_offset)
                  samples.append(offset_result.time_offset_ms.mean)
                  await asyncio.sleep(0.2)
              phone_to_pc_time_offset_ms = sum(samples) / len(samples)
              print(f"Phone-to-PC time offset (averaged): {phone_to_pc_time_offset_ms} ms")
          except Exception as e:
