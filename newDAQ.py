...
async def start_recording_and_trial():
    ...
    # ── STEP 1: Measure time offset BEFORE any recording starts ──
    # Do this first while network is idle for best accuracy
    ...
    if neon_device:
        try:
            ...
            print(f"Phone-to-PC time offset (pre-recording): {phone_to_pc_time_offset_ms} ms")
        except Exception as e:
            print(f"Could not get time offset: {e}")
            phone_to_pc_time_offset_ms = None
    
    # ── STEP 2: Start QTM and capture PC clock at that moment ──
    ...
    # ── STEP 3: Wait 3 seconds, then start Neon ──
    # Record this delay explicitly so analysis can account for it
    NEON_DELAY_MS = 3000.0
    await asyncio.sleep(NEON_DELAY_MS / 1000.0)
    
    pc_time_neon_start_ms = None
    if neon_device:
        try:
            await asyncio.to_thread(neon_device.recording_start)
            pc_time_neon_start_ms = time.time() * 1000
            print(f"Neon recording started at PC time: {pc_time_neon_start_ms}")
        except Exception as e:
...
def on_stop_trial_button():
    ...
    threading.Thread(target=lambda: asyncio.run(stop_neon_recording()), daemon=True).start()
